...
extern "C" void initialize_volume_render(RCamera& sceneCam, const Grid& sdf, const int& num_sdf, const std::vector<VoxelMaterial>& materials, const RenderingSettings& render_settings,
	const SceneSettings& scene_settings, const RPerlinNoise& nosie);
...
namespace RayEngine
{
	int mainss(MainWindow *main_window);
	double currentFrame;
	double lastFrame;
	double deltaTime;
	float click_timer;
	...
	MainWindow::MainWindow()
	{
		...
		Grid distance_field(std::string("SDFs/Edited.rsdf"));
		...
		initialize_volume_render(*SceneCam, distance_field, 1, materials, render_settings, scene_settings, noise);
#endif
	...
	void MainWindow::RenderFrame()
	{
		...
		// clear image
		//cudaMemsetAsync(d_output, 0, SCR_WIDTH * SCR_HEIGHT * 4);
	...
	void MainWindow::processInput(float delta_time, GLFWwindow* window)
	{
		...
		if (!show_mouse)
		{
			...
			if (render_settings.gravity)
			{
				bool overlaps = sdf_collision(tmp_ca);
				if (!overlaps && render_settings.gravity)
	...
	int mainss(MainWindow *main_window)
	{
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			main_window->main_ui->render_main_hud(show_mouse, brush, character_speed, render_settings, scene_settings, main_window->SceneCam->campos);
			glfwSwapBuffers(window);
