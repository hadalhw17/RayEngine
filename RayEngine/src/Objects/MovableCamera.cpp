...
void RMovableCamera::strafe(const double& scale)
{
...
float rad_to_deg(const double& radians) {
	float degrees = radians * 180.0f / M_PI;
	return degrees;
}

float deg_to_rad(const double& degrees) {
	float radians = degrees / 180.0f * M_PI;
	return radians;
}

void RMovableCamera::set_fovx(const double& fovx)
{
	fov.x = fovx;
	fov.y = rad_to_deg(atanf(tanf(deg_to_rad(fovx) * 0.5f) * (SCR_HEIGHT / SCR_WIDTH)) * 2.f);
}

void RMovableCamera::change_yaw(const double& scale)
{
	yaw += scale;
	fix_yaw();
}
...
void RMovableCamera::move_forward(const double& scale)
{
	position += view_direction * scale;
}

void RMovableCamera::change_pitch(const double& scale)
{
	pitch += scale;
	fix_pitch();
}

void RMovableCamera::rotate_right(const double& scale)
{
	float yaw2 = yaw;
	yaw2 += scale;
	float pitch2 = pitch;
...
void RMovableCamera::change_radius(const double& scale)
{
	radius += scale;
	fix_radius();
}

void RMovableCamera::change_altitude(const double& scale)
{
	position += view_direction * scale;
}

void RMovableCamera::set_resolution(const double& x, const double& y)
{
}

void RMovableCamera::change_focial__distance(const double& scale)
{
	focial_distance += scale;
	fix_focial_distance();
}

void RMovableCamera::change_aperture_diameter(const double& scale)
{
