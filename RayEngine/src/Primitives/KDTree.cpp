...
RKDTreeCPU::RKDTreeCPU(float3* _verts, uint3* _faces, float3* _norms, float2* _uvs, int num_verts, int num_faces, int num_norms, int num_uvs)
{
	...
	this->faces = new uint3[num_faces];
...
bool RKDTreeCPU::intersect(KDNodeCPU* node, RRay r, float& t, float3& normal)
{
	...
	if (node->box.intersect(r, tNear, tFar))
	{
		...
		else
		{
			...
			{
				uint3 tri = faces[node->objIndeces[i]];
				float3 v0 = verts[(int)tri.x];
				float3 v1 = verts[(int)tri.y];
				float3 v2 = verts[(int)tri.z];
