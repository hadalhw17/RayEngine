	...
	float3* sdf_max, float3 light, float4& final_colour, int shape)
{
	...
	while (t < light_dst)
	{
		...
		for (int sdf_c = 0; sdf_c < num_sdf; ++sdf_c)
		{
			GPUVolumeObjectInstance curr_obj = instances[sdf_c];
			float siny = sinf(curr_obj.rotation.y);
			float cosy = cosf(curr_obj.rotation.y);
			...
			new_o += curr_obj.location;

			float3 transformed_from = new_o + t * new_dir;
			HitResult transformed_hit_result;
			transformed_hit_result.ray_o = new_o;
			transformed_hit_result.ray_dir = new_dir;
			float d = 0;
			if (curr_obj.index == -1)
			{
				d = sphere_distance(transformed_from, 1);
			}
			else
			{
		...
		res = fminf(res, 32 * min_dist / t);
		t += min_dist;
		++step_count;
	}
	final_colour += res * (lightInt * dot(hit_result.ray_dir, normal));
	final_colour = clip(final_colour);
	return;
}
 ...
 int num_sdf, float3 * step, int3 * dim, int curr_sdf, float4 & final_colour, int step_count)
{
	...
	if (curr_obj.index != -1)
	{
		...
		{
			//sphere_trace_shadow(scene, instance, p_hit, n, volumes, num_sdf, step, dim, sdf_max, lights[i], final_colour, curr_sdf);
		}
	}
...
// Bind triangles to texture memory
////////////////////////////////////////////////////
void bind_scene_to_texture(float2* dev_sdf_p, int3 dim)
{
	cudaTextureDesc tex_desc;
	...
	tex_desc.addressMode[2] = cudaAddressModeWrap;    // wrap texture coordinates
	tex_desc.readMode = cudaReadModeElementType;

	//cudaMemset(&tex_desc, 0, sizeof(tex_desc));
	scene_texture.normalized = false;                      // access with normalized texture coordinates
	scene_texture.filterMode = cudaFilterModeLinear;        // Point mode, so no 
	scene_texture.addressMode[0] = cudaAddressModeBorder;    // wrap texture coordinates
	scene_texture.addressMode[1] = cudaAddressModeBorder;    // wrap texture coordinates
	scene_texture.addressMode[2] = cudaAddressModeBorder;    // wrap texture coordinates
	cudaArray* d_sdf_array;

	cudaChannelFormatDesc* arr_channelDesc = &cudaCreateChannelDesc<float2>();
	cudaMalloc3DArray(&d_sdf_array, arr_channelDesc, make_cudaExtent(dim.x * sizeof(float2), dim.y, dim.z), 0);

	cudaMemcpy3DParms copyParams = { 0 };

	//Array creation
	copyParams.srcPtr = make_cudaPitchedPtr(dev_sdf_p, dim.x * sizeof(float2), dim.y, dim.z);
	copyParams.dstArray = d_sdf_array;
	...
	//channelDesc->f = cudaChannelFormatKindFloat;
	cudaBindTextureToArray((const textureReference*)& scene_texture, d_sdf_array, channelDesc);
	//cudaCreateTextureObject(tex_sdf, &res_desc, &tex_desc, NULL);
...
uchar4* initialize_volume_render(RCamera sceneCam, Grid* sdf, int num_sdf)
{
	...
	volume_objs[1] = GPUVolumeObjectInstance(-1, make_float3(2, 5, 0), make_float3(0));
	d_num_instances = 2;
